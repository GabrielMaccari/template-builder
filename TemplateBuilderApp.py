...
class MyApp(QWidget):
    def __init__(self):
        ...
        self.labels = [self.stPonto, self.stUTM_E, self.stUTM_N, self.stAltitude, self.stDisciplina, self.stData,
                       self.stEquipe, self.stToponimia, self.stPonto_de_controle, self.stUnidade,
                       self.stUnidade_litoestratigrafica, self.stTipo_de_afloramento, self.stIn_situ,
                       self.stGrau_de_intemperismo, self.stNumero_de_amostras, self.stPossui_croquis,
                       self.stPossui_fotos]
    ...
    def load_dependency(self, path, file_type):
        try:
            ...
            msg = QMessageBox(parent=self,
                              text=f'Um dos arquivos necessários para o funcionamento da ferramenta está faltando ({path}). Restaure o arquivo em questão e tente novamente.\n\n{erro}')
    ...
    def open_file(self):
        file = QFileDialog.getOpenFileName(window, caption='Selecione uma tabela .xlsx contendo os dados de entrada.',
                                           directory=os_getcwd(), filter='*.xlsx', initialFilter='*.xlsx')
        ...
        if self.file_path != '':
            try:
                ...
                self.df.drop(remove_cols, axis='columns',
                             inplace=True)  # self.df = self.df.loc[:, ~self.df.columns.str.contains('^Unnamed')]
        ...
        for c in e_columns:
            ...
            if self.columns_ok:
                self.readyLabel.setText(
                    'Clique no botão abaixo para montar o template da caderneta com os dados carregados.')
                self.readyLabel.setStyleSheet('QLabel {color: green}')
                self.buildButton.setEnabled(True)
            else:
                self.readyLabel.setText(
                    'Corrija na tabela os problemas indicados acima e carregue novamente o arquivo.')
    ...
    def build_template(self):
        ...
        for i in range(0, 9): document.add_paragraph(text='', style=normal)
        document.add_paragraph(text='Caderneta de Campo Compilada'.upper(), style=title)
        for i in range(0, 2): document.add_paragraph(text='', style=normal)
        document.add_paragraph(text='Mapeamento Geológico UFSC'.upper(), style=info_title)
        for i in range(0, 2): document.add_paragraph(text='', style=normal)

        info_list = ['Projeto:'.upper(), 'Ano:'.upper(), 'Professores responsáveis:'.upper(),
                     'Número da área/faixa:'.upper(), 'Integrantes do grupo:'.upper()]

        for info in info_list:
            document.add_paragraph(text=info, style=info_title)
            document.add_paragraph(text='<preencha aqui>'.upper(), style=info_text)
        ...
        for point in dataframe.itertuples():
            try:
                ...
                table_data = {'Data:'.upper(): point.Data if str(point.Data) != 'NaT' else '-',
                              'Coordenadas:'.upper(): ('%.0f E / %.0f N' % (point.UTM_E, point.UTM_N) if (str(point.UTM_E) != 'nan' and str(point.UTM_N) != 'nan') else '______ E / _______ N'),
                              'Altitude:'.upper(): ('%.0f m' % (point.Altitude) if str(point.Altitude) != 'nan' else '-'),
                              'Equipe:'.upper(): point.Equipe,
                              'Ponto de controle:'.upper(): point.Ponto_de_controle,
                              'Tipo de afloramento:'.upper(): point.Tipo_de_afloramento,
                              'In situ:'.upper(): point.In_situ,
                              'Grau de intemperismo:'.upper(): point.Grau_de_intemperismo,
                              'Amostras:'.upper(): ('%d' % (point.Numero_de_amostras)) if (str(point.Numero_de_amostras).isnumeric() and point.Numero_de_amostras != 0) else '-',
                              'Unidade:'.upper(): (str(point.Unidade) + ' - ' + str(point.Unidade_litoestratigrafica)) if (str(point.Unidade) != 'nan' and str(point.Unidade_litoestratigrafica) != 'nan') else '-'
                              }
                ...
                document.add_paragraph(text='Toponímia'.upper(), style=subtitle)
                document.add_paragraph(text=str(point.Toponimia if str(point.Toponimia) != 'nan' else '-'),
                                       style=normal)

                # Adiciona a seção de descrição do ponto
                document.add_paragraph(text='Descrição'.upper(), style=subtitle)
                document.add_paragraph(text="...", style=normal)
                ...
                try:
                    ...
                    if sample_qty > 0:
                        document.add_paragraph(text='Amostras'.upper(), style=subtitle)
                ...
                for i in range(18,
                               len(column_list) + 1):  # Itera em todas as possíveis colunas de medidas (18-x*)   *x ≤ 31 se ninguém fez merda
                    if str(point[i]) != '' and str(point[i]) != 'nan':
                        hasMeasurements = True
                        column = str(column_list[
                                         i - 1])  # i-1 porque na lista começa em zero, enquanto que no dataframe começa em 1 (o zero é o id)
                ...
                if hasMeasurements:
                    document.add_paragraph(text='Medidas estruturais'.upper(), style=subtitle)
                ...
                if str(point.Possui_croquis) != 'Não' and str(point.Possui_croquis) != '0':
                    document.add_paragraph(text='Croquis'.upper(), style=subtitle)
                    document.add_paragraph(text='...', style=normal)

                # Adiciona a seção de fotos, se houver alguma
                if str(point.Possui_fotos) != 'Não' and str(point.Possui_fotos) != '0':
                    document.add_paragraph(text='Fotos'.upper(), style=subtitle)
                    document.add_paragraph(text='...', style=normal)
            ...
            # Caso haja algum problema ao montar o template, tenta identificar em qual ponto está o problema
                ...
                if ponto != 'undefined':
                    msg = QMessageBox(parent=self,
                                      text=f'Ocorreu um erro ao montar o template. Verifique e corrija os dados do ponto {ponto} e tente novamente.\n\n{erro}')
                else:
                    msg = QMessageBox(parent=self,
                                      text=f'Ocorreu um erro ao montar o template. Verifique os dados e tente novamente.\n\n{erro}')
    ...
    def save_file(self, document):
        ...
        self.outFile = QFileDialog.getSaveFileName(self, 'Salvar template', str(os_getcwd()),
        ...
        if output_path != '':
            try:
                ...
                msg = QMessageBox(parent=self,
                                  text=f'Template criado com sucesso! O arquivo foi salvo em "{output_path}".')
                msg.setWindowTitle("Sucesso")
            except:
                msg = QMessageBox(parent=self,
                                  text=f'Ocorreu um erro ao salvar o arquivo. Verifique se "{output_path}" não está aberto em outro programa e se você possui permissão para salvar nessa pasta, e então tente novamente.')
